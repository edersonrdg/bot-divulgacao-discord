...
module.exports.run = async (client, msg, args) => {
    ...
    // Ids que podem utilizar o comando
    ...
    function createnewembed(courrentDivOn, courrentDivDnd, courrentDivIdle) {
    ...
    await msg.channel.send(createnewembed(courrentDivOn, courrentDivDnd, courrentDivIdle)).then(message => {
        const t = setInterval(() => {
            if (divComlete == true) {
                clearInterval(t)
                return message.edit(createnewembed(courrentDivOn, courrentDivDnd, courrentDivIdle)).then(res => {
                    res.delete({ timeout: 20000 })
                })
            }
            message.edit(createnewembed(courrentDivOn, courrentDivDnd, courrentDivIdle))
        }, 3000)
    })
};